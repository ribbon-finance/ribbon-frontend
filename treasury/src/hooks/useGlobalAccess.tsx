...
import { initialAccessState, useGlobalAccessState, useWebappGlobalState } from "../store/store";
...
const useGlobalAccess = () => {
  ...
  const [globalAccess, setGlobalAccess] =
    useGlobalAccessState("access");
      ...
      if (vault) {
        setGlobalAccess((accessState) => {
          if (!accessState.includes(vault as TreasuryVaultOptions)) {
            localStorage.setItem("auth", JSON.stringify([...accessState, vault as TreasuryVaultOptions]))
